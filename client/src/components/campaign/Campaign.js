...
function Campaign() {
  ...
  const ArticleCard = ({ id, thumbnail, title, description, rewards, tags, status, onclick }) => {    
    return (
      <Card key={id} onClick={onclick} className="pointer" style={{marginBottom: "20px", position: "relative"}}>
        <div style={{position: "relative"}}>
          {
            status === 0 ? (
              <Badge bg="secondary" style={{position: "absolute", top: "10px", right: "10px"}}>Draft</Badge>
            ) : 
            (
              <Badge bg="success" style={{position: "absolute", top: "10px", right: "10px"}}>Published</Badge>
            )
